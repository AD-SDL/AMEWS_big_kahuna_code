...
def LS_only_test():
    ...
    ld.pt.add(
        "plate2", "Rack 3x4 six Kaufmann H-cells", "Deck 14-15 Heat-Stir 1"
    )  # to stir all six cells
...
def LS_iterative():  # a test of LS functions - a cyclical walk through plate1 mapping it on ICP1 rack
    ...
    ld.as_finish()
    if flag:
        ld.smtp.alert("run %d completed" % (ld.ID))
...
def run_tests():
    ...
    if "no-go" in ld.as_execute_wDC(739):
        sys.exit()
