...
from CustomDAQ import check_BK_door
...
class CustomLS10:  # LS API wrapper calls
    ...
    def add_chem(
        ...
        if chem:  # chemical
            ...
            if not elements:
                ...
                self.tracker.add(
                    ID, name=chem, elements=elements, concentrations=concentrations
                )
    ...
    def single_well_transfer(
        ...
        if add_from not in self.sources:
            if self.add_chem(add_from, None):
                pass  # add substrate to sources
    ...
    def sources_df(self):  # make a dataframe for the sourcing map
        if self.chem:
            self.sdf = pd.DataFrame.from_dict(
                self.chem,
                orient="index",
                columns=[
                    "chemical",
    ...
    def sources_csv(self):  # convert sourcing dataframe to CSV file
        if len(self.sdf):
            self.sdf.to_csv("sourcing_%d_%s.csv" % (self.ID, self.stamp), index=False)
            if self.verbose:
    ...
    def log_libs(self):
        ...
        if libs:
            for lib in libs:
                ...
                if self.verbose:
                    print(
                        "added library %d :: %s, %dx%d :: color = %d (%s)"
                        % (
                            ID,
                            lib.Name,
                            lib.Rows,
