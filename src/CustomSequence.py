...
class CustomSequence:
    ...
    def select_dir(self):  # select directory by cursor
        ...
        self.exp = filedialog.askdirectory(
            initialdir=self.exp_path, title="Select an experiment"
        )
    ...
    def consolidate_waste_logs(
        self,
    ):  # consolidate waste logs and remove individual logs
        ...
        for f in files:
            if "combined" not in f:
                ...
                try:
                    ...
                    df = df[
                        ~df["container"].str.contains("glass", case=False, na=False)
                    ]
                    if len(df):
                        dfs.append(df)
                    # os.remove(f)
    ...
    def combine_AS_digests(
        self, log="AS_sequence_log", digest="ASDigest_vols_combined"
    ):
        ...
        for i, r in self.log.iterrows():
            ...
            if f:
                ...
                if len(df):
                    print(
                        ">> Processed %s : ID=%d, container=%s, barcode=%d, category=%s"
                        % (f, ID, container, barcode, category)
                    )
            ...
            for i, r in self.last_digest.iterrows():  # zero times from the first fill
                ...
                if "load" in r["category"]:
                    ...
                    if "source" in r["plate from"]:
                        ...
                        self.last_digest.loc[i, "category"] = r["category"].replace[
                            "fill", "calibrate"
                        ]
                else:
                    self.last_digest.loc[i, "sample"] = "%s-BK-%s" % (
                        r["container"],
                        r["well to"],
                    )
                print(c)
    ...
    def add_ICP_records(self, log="AS_sequence_log", digest="ASDigest_vols_combined"):
        ...
        for _, r in self.log.iterrows():
            ...
            if "rack" in r["category"]:
                files = glob(
                    os.path.join(self.exp, "run_%s_*_converted.csv" % container)
                )
                if files:
                    print(
                        "\n>> Processing container %s: %d file(s))"
                        % (container, len(files))
                    )
                    for f in files:
                        ...
                        if flag == 0:
                            ...
                            for col in df.columns:
                                if (
                                    col != "Sample"
                                    and col not in self.last_digest.columns
                                ):
        ...
        self.last_digest = self.last_digest[
            ~self.last_digest["category"].str.contains(
                "fill|load", case=False, na=False
            )
        ]
        self.last_digest = self.last_digest.drop(
            columns=[
                "datetime",
                "container",
                "library",
        ...
        if any("address" in col for col in self.last_digest.columns):
            self.last_digest = self.last_digest.drop(
                columns=["address from", "address to"]
            )
