...
class AMEWS:
    ...
    def __init__(self):
        ...
        self.counters = [self.cell2counter(x) for x in self.cells] # green/feed compartments in each cell
        print("\n>> Cells, sample compartments (rewd): %s" % self.cells)
        print(">> Cells, feed compartments (green): %s\n" % self.counters)
        self.cell_rack = "Rack 4x2 Mina H-cell"
        self.cell_map= { "plate1" : "Deck 12-13 Heat-Cool-Stir 1",
    ...
    def find_last_cell(self):
        ...
        print(">> Last probed cell is %s, %s => index %d" % (plate, well, 1 + self.last_cell))
        self.last_cell += 1
...
##########################################  the common opening steps #####################################################
    ...
    def LS_open(self, cells, name = "sampling"): 
        ...
        if "load" in name: code = "Etip_WS" # "Etip" for cell filling with the solvent
        else: code = "skip" # all other LS methods
...
################################################## experiment sequence ###########################################################################
    ...
    def LS_load(self): # loading cells with a standard
       ...
       if self.ld.finish()==0: # writing into the database and creating log files and xml files for AS
                     sys.exit(0) 
    ...
    def LS_fill_calibrate(self):
        ...
        for plate, well in u:
                ...
                if col in self.ld.input_names:
                    for _, row in df.iterrows():
                        ...
                        if volume and plate == row["plate"].strip() and well ==  row["well"].strip():
                                ...
                                self.ld.dispense_chem(col, # cell fill
                                    plate, 
                                    well,
                                    volume, # added volume
            ...
            if self.calibrate: 
                ...
                print("\n>> Transfered calibration volume %1.f uL and %.1f uL chaser to ICP1 well %s" % (self.calibrate, 
                                                                                                       self.chaser, 
                                                                                                       well_to)
                      )
...
#=============================================================================================================
    ...
    def LS_calibrate(self):
        ...
        self.ld.transfer_replace_mapping(self.calibrate, # volumes
                                        self.chaser, # chaser volume
                                        "1tip"
                                        ) 
