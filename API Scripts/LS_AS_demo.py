...
#    ld.dummy_fill("plate1", 5e4, "offdeck") # 50 mL
    ...
    ld.transfer_replace_mapping(250, # volume
                                2000, # chaser volume
                                "1tip",
                                0., #0.01 # min
                                ) 
...
def LS_only_calibration():
    ...
    for i in range(ld.tm.n_from):   
        ...
        ld.dispense_chem("mixture%d" % (i+1),
                         plate,
                         well,
                         1000, # added volume
                         "skip", # "1tip", # actual addition
    ...
    ld.transfer_replace_general(volumes, # volumes
                                    2000, # chaser volume
                                    "1tip",
                                    0., #0.01 # min
                                    ) 
...
def run_tests():
     ...
     if "no-go" in ld.as_execute_noDC(808,0): sys.exit()
