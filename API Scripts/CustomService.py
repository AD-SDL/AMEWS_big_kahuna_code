...
import sys, os, string, math, re, random,  copy
import json, time, clr, shutil, zipfile
...
class CustomLS10: # LS API wrapper calls
    ...
    def __init__(self):
        ...
        self.smtp = CustomAlert()     # SMTP server for sending AS alerts   
        self.smtp.instrument = "BK_AARL200@anl.gov"
        self.stamp = self.tstamp()     # time stamp for all files related to the LS design
        ...
        # chemfile and prompts file
        self.path = os.getcwd() # assumes resources in the current directory
        print("writes ChemicalManager and prompt XML files in %s" % self.path)
        self.chemfile = ChemFile()
        self.promptsfile = PromptsFile()
        self._prompts = os.path.join(self.path,"promptsWithDC.xml") # default for using Design Creator
        self._chem ="" # default for using Design creator
        
        # LS API settings
        self.cmap = plt.get_cmap("nipy_spectral")  # color map
        self.units = "ul" # units - must be in lower case unless capitalized in the unit table
        ...
        self.TAGS = {"skip":"SkipMap",                          # short codes for common LS design tags
        ...
        self.PTYPES = ["Temperature", "Time", "Rate", "Number", 
                       "Text", "Stir Rate", "Temperature Rate"] # allowed parameter types
        
        # numerical codes for pause actions
        # code book for text parameter maps => AS Pause messages
        self.pause_codebook = { 1000: "Remove ICP rack",
                        1100: "Remove and replace ICP rack",
                        1200: "New ICP rack",
                        1300: "Wait for external resume",
                        1400: "Send signal and wait for response",
                        10: "Stirring On",
                        20: "Stirring Off"
                        }
    ...
    def add_chem(self,
        ...
        if chem: # chemical
              ...
              if source: ID = "%s:%s" % (source, well)
              else: ID = chem

              if not elements: 
                  self.tracker.add(ID, name=chem)
              else:
                  self.tracker.add(ID, name=chem, 
                                   elements=elements, 
    ...
    def dispense_chem(self, # dispenses chemical from a source & makes a source map
        ...
        if "Chaser" not in tag: 
            ...
            if source: component_ID = "%s:%s" % (source, source_well)
            else: component_ID = chem
        ...
        if opt and layerIdx < 0: 
           self.chem[self.map_count] = (chem, source, source_well, add_to, range_str, volume)
    ...
    def single_well_transfer(self, # single cell transfer between the substrates   
        ...
        if add_from not in self.sources: 
            if self.add_chem(add_from, None): pass # add substrate to sources   
        ...
        component_ID = "%s:%s" % (add_from, well_from)

        self.tracker.add(ID, component_ID,  volume)

        if self.chaser: 
            self.tracker.add(ID, "solvent",  self.chaser)
        self.chaser = 0
        ...
        self.map_substrates[self.map_count] = [add_from, add_to]
    ...
    def region_fill(self, plate, volume=50000, what="solvent", region="full"): # skipped dummy fill
        if self.verbose: 
            print("\nFictitious fill of %s (%s) with %f uL of %s" % (plate, region, volume, what))
        if plate in self.pt.plates:
            if region=="full": 
    ...
    def modify_tag_code(self, code, letter): # add a letter coded option to the tag code, can be more than one letter
         if '_' in code:
    ...
    def transfer_replace_well(self, # add chaser, aliquot, replace it with solvent & delay
        ...
        if chaser>0: 
            self.dispense_chem("solvent" ,add_to, well_to, chaser, "chaser_S") # changed 1-30-2025 was _from
            self.chaser = chaser

        i = self.single_well_transfer(add_from, add_to, well_from, well_to, volume, tag_code)

        self.dispense_chem("solvent", 
                           add_from, 
                           well_from, 
                           volume, 
    ...
    def transfer_replace_mapping(self, volume, chaser, tag_code="1tip", d=0.0): # do the transfer sequence for the entire transfer map
        volumes = [volume] * len(self.tm.mapping)
        self.transfer_replace_general(volumes, chaser, tag_code, d)


    def transfer_replace_general(self, volumes, chaser, tag_code="1tip", d=0.0): # do the transfer sequence for the entire transfer map
          i=0 
          ...
          for _, add_from, well_from, _, add_to, well_to, _ in self.tm.mapping:
              self.transfer_replace_well(add_from, add_to, well_from, well_to, volumes[i], chaser, tag_code, d)
    ...
    def finish(self): # finish design     
        ...
        if self.error_message: return 0

        self.ID_folder()
        self.pt.to_df()
        
        self.pt.to_csv(os.path.join(self.dir,"substrates_%d" % self.ID), 
                       self.stamp) # save the substrate map as csv dataframe file
        ...
        self.waste.to_csv(os.path.join(self.dir,"waste_ondeck_%d.csv" % self.ID), index=True)
    ...
    def finish_iterative(self): # finish design, iterative calls
        ...
        if self.ID<0:
            ...
            self.to_file(os.path.join(self.dir,"%s_%d_step%d.lsr" % (self.name,self.ID,self.transfer)))
    ...
    def as_execute(self, opt = 0):  # 1,3 no reinitialization of AS clients, 0,2 - no finishing
        if opt == 0 or opt==2: # initialization of AS clients
            if not self.as_prep(): return "no-go"    # prepare AS SiLA client
    ...
    def as_prep(self): # prepare for run, return 1 if ok, -1 if door is not interlocked, 0 if error
        # check door state
    ...
    def as_run_paused(self): # begin the standard experiment, stop on pauses
        self.as_state =  self.as10.run(self.ID, 
                      self._prompts, 
                      self._chem,
                      None, # tip file
                      True,  # until paused
    ...
    def as_run_resume(self, opt): # resume the experiment, True - until paused
        self.as_state =  self.as10.run(self.ID, # does not matter
    ...
    def combine_files(self,kind,combined): # combine in the order of creation dates, use wildtypes for kind
...
################################## working with containers #####################################################
    ...
    def make_container(self, plate, new_type = ""): # creates container object for export, json format
    ...
    def supply_request(self, plate, rack_type): # creates request for container object, json format
         kind, position, rows, cols = self.pt.get(plate)
         dt = datetime.now()
         self.last_code = self.disp.datetime2abcd(dt) 
         self.last_barcode = ""
    ...
    def trash_request(self, plate, rack_type): # creates container object for trash, json format
         kind, position, rows, cols = self.pt.get(plate)
         dt = datetime.now()
         self.last_code = self.disp.datetime2abcd(dt)
        ...
        try:    
            ...
            for  _, row in df.iterrows():
                ...
                if plate in self.pt.plates and "source" in plate:
                    ...
                    name = row["name"].strip() 
                    if name in self.input_names: # source chemicals need to have unique names
                    ...
                    if r<=rows and c<=cols:
                        ...
                        print(">> Impossible well %s for %s, %s, abort" % (position, plate, kind))
                        print(" --- well (%d,%d) in a %dx%d rack !" % (r, c, rows, cols))
                        sys.exit(0)
                else:
            ...
            if self.tm.in_lib(self.tm.lib_from, plate, well):
                ...
                for col in df.columns:
                    if col in self.input_names:
                        for _, row in df.iterrows():
                            volume = float(row[col])
                            if volume and plate == row["plate"].strip() and well ==  row["well"].strip():
                                    total += volume
                                    self.dispense_chem(col,
                                        plate, 
                                        well,
...
class CustomTransferMap: # a class to create transfer maps
    ...
    def generate_combined_labels(self, pt, plate, ranges_str, direction, offset=0): # combines and orders well ranges 
        # added option offset 
    ...
    def add_from(self,pt,plate,reagent_str,direction,offset=0): # 0 by column, 1 by row # a list of sampled wells
        self.lib_from += self.generate_combined_labels(pt,plate,reagent_str,direction,offset)
        self.n_from=len(self.lib_from)

    def add_to(self,pt,plate,reagent_str,direction,offset=0): # 0 by column, 1 by row # a list of destination wells
                                                              # added optional offset in the well list
        self.lib_to +=self.generate_combined_labels(pt,plate,reagent_str,direction,offset) 
