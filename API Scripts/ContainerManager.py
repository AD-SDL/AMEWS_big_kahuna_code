import sys, os, math, re, copy, glob, random
import json, time, string, shutil, keyboard, subprocess
import pandas as pd
import numpy as np
from datetime import datetime, timedelta
import cv2
from pyparsing import Opt # for capturing images using OpenCV
...
class CustomBarcode:
    ...
    def check_camera(self, camera=0): # check there is a camera
        ...
        if self.cam.isOpened(): 
            ...
            print("\n>> Camera%d : %s, %d x %d, %d frames/s" % (camera, name, width, height, fps))
            return True
        else: 
            return False
    ...
    def snapshot(self, camera=0, focus=100): #take snapshot
        ...
        if self.check_camera(camera):
            ...
            self.cam.set(cv2.CAP_PROP_FOCUS, focus) # change focus, must be a multiple of 5
            time.sleep(1)
...
################################ container encoding date time => four alphanumerical symbols ############################
    ...
    def encode(self, n):
        ...
        if self.verbose ==2:
            ...
            print("--- %d seconds since the beginning of the year => code = %s\n" % (m, s))
        self.COUNT+=1
        return s
    ...
    def datetime2abcd(self, dt): # minutes since the beginning of a year to 4 alphanumeric symbols + count module alphabet length
    ...
    def save(self, container=None, stamp=True): # timestamps, codes, and saves container object, json format - creation of the object
        if container is None:
            container = self.container
        if stamp: 
            dt = datetime.now()
            container["creator"]["datetime"] = str(dt)           
        print(">> Saved %s container %s on %s" % (container["route"]["type"], 
                                                  container["code"], 
                                                  dt))       
...
########################### container queueing #######################################################################
    ...
    def service_request(self, code): # requestuing service by activation of "ready" code word
            ...
            if os.path.getsize(s):
                with open(s, 'r') as f:
                    ...
                    if c: 
                        ...
                        if match: # make sure that the code corresponds to the file name
                            code  = match.group(1)
                            if c["code"] != code:  
                                c["code"] = code
                                self.update(c)
        ...
        if len(fs): 
            for s in fs:
                if os.path.getsize(s) == 0:
                    self.force_remove(s)
                    n+=1
        if n: print(">> Purged %d empty containers" % len(fs))
    ...
    def process_container(self, c): # process container
        ...
        if c["route"]["ready"] == "yes":
            ...
            print("--- Classifier: container %s -> current instrument %s" % (code, instrument))
    ...
    def notinqueue(self, q, code): # checks that container <code> is in the instrument queue <q>
        try: 
            for record in q["queue"]:
                if "code" in record and record["code"] == code: return False
        except Exception as e:
    ...
    def load_queue(self, instrument, keep=True): # load instrument queue
        ...
        try: 
            ...
            print(">> Error %s: cannot load queue for instrument %s, skip" % (e, instrument))
            self.queue[instrument] = {"queue" : []}
            
        return self.queue[instrument]
            
    def remove_task(self, instrument, task=None, index=True): # remove task from queue, optionally index step, if task is None, remove the first task
        t = 0
        if instrument not in self.queue: return
        q = self.queue[instrument]["queue"]
        if len(q) == 0: return
        if task: t = q.index(task)
        code = q[t]["code"]
        del q[t]

        if self.verbose: 
            print(">> Removed task %d (container %s) from instrument %s queue" % (t, code, instrument))
            
        c = self.load(code)
        c["route"]["ready"] = "yes"
        if index: self.index(c)  
    ...
    def trash_container(self, c): # move container to trash folder and create waste bill
        dt = datetime.now()
        code = c["code"]     
        ...
        if os.path.exists(old):
            ...
            try:
               # if not os.path.exists(new):
                    ...
                    waste.to_csv(os.path.join(self.TRASH,
                                            "waste_%s_%s.csv" % (code,position)), 
                                            index=True)                    
            except Exception as e:
...
#        
    def FIFO_UR5(self): # FIFO service in UR5 queue
        ...
        if c:
            code = c["code"]
            if self.verbose:
                print("\n>> Processing container %s in UR5 queue, priority = %s" % (code, task["priority"]))
            ...
            if self.pick_UR5(c, i_pick, f_pick):
               ...
               self.smtp.alert(">> UR5 is stack picking container %s, abort" % code, importance="High")
               return 4 # termination 

            if self.drop_UR5(c, row):
               print(">> UR5 is stack picking container %s, abort" % code)
               self.smtp.alert(">> UR5 is stack dropping container %s, abort" % code, importance="High")
               return 4 # termination 
    ...
    def check_queue(self, instrument):
        ...
        if q:
            q = q["queue"]
            if len(q) == 0:  return None, None
            flag=1
            for task in q:
                if task["priority"] == "high": # high priority tasks are selected FIFO
                    flag = 0
                    break
            if flag: task = random.choice(q)      # low priority tasks are selected at random       
            try: 
                if task: 
    ...
    def update_UR5(self, c): # update UR5 status file
         ...
         for i, row in self.UR5.iterrows():
            if  instrument == row["instrument"] and rack in row["types"] and position == row["position"]:
    ...
    def update_UR5_line(self, c, i): # updates UR5 status file line <i>
        ...
        dt = datetime.now()
        if self. verbose: 
            print(">> Updating UR5_status for rack=%s, instrument=%s, position=%s" % (rack, instrument, position))
    ...
    def load_UR5_status(self, keep): # load UR5 protocols
        ...
        try: 
            ...
            if not keep:
                self.UR5['comment'] = ''
                self.UR5['stamp'] = ''
                self.UR5['status'] = self.UR5['status'].apply(lambda x: '' if x != 'supply' else x)
    ...
    def pick_UR5(self, c, i, flag):        
       ...
       protocol = "pick_%s_%s" % (self.UR5.loc[i, "instrument"], 
    ...
    def pick_UR5_check(self, c):
        ...
        if self.verbose: print("\n>> Check pick: rack=%s, service=%s, instrument=%s, position=%s" % (rack, 
                                                                                  service, 
                                                                                  instrument, 
                                                                                  position))
        
        if service == "trash":  return -1, 1                  
        for i, row in self.UR5.iterrows():
        ...
        if service == "supplies":
           ...
           if self.supplies==0:
            ...
            print(">> Cannot find UR5 protocol for instrument %s, picking rack %s" % (instrument, rack))
    ...
    def drop_UR5_check(self, c):
        ...
        if self.verbose: print(">> Check drop: rack=%s, service=%s, instrument=%s" % (rack, 
                                                                                  service, 
                                                                                  instrument))
        ...
        if flag and instrument != "Storage": # requested instrument is busy, place in storage
            if self.verbose: print("--- checks storage")
            flag = 0
            for i, row in d.iterrows():
                if row["instrument"] == "Storage": 
                    flag = 0 
                    print(">> Instrument %s unavailable, placed rack %s in storage" % (instrument, rack))                   
                    return i, row, 1
                
        if flag and c["route"]["priority"] == "clear": # storage is full
                                                       # place in trash if priority is clear, do noting otherwise 
            
            if self.verbose: print("--- checks trash")
            for i, row in d.iterrows():
                if row["instrument"] == "Trash":
                    print(">> Expected placement unavailable, trashing")
                    self.smtp.alert("Instrument & storage unavailable: forced removal of rack %s" % (instrument))
    ...
    def supplies_check(self):
        ...
        if count==0 and self.supplies==0:
            self.supplies = 1 
            print("\n****************  ALERT: ran out of supplies ****************", importance="High")
            self.smtp.alert("run out of supplies")
        else: self.supplies=0 
    ...
    def trash_check(self):
        ...
        df =self.UR5.copy()     
        df = df[(df['status'] == "") & (df['instrument'].str.contains('Trash', case=False))]
        count = df.shape[0]
    ...
    def storage_check(self):
        ...
        df =self.UR5.copy()         
        df = df[(df['status'] == "") & (df['instrument'].str.contains('Storage', case=False))]
        count = df.shape[0]
