...
import sys, os, json, math, string
import pandas as pd
import numpy as np
...
class CustomTracker:
    ...
    def __init__(self):
        ...
        self.dir = "C:/Users/%s/Dropbox/Instruments cloud/Robotics/composition tracker" % self.user
    ...
    def add_composition(self, ID, component_ID, volume): # composition in primary compounds after addition
    ...
    def source_constitution(self, elements, concentrations): # "element" constitution in unirs
        constitution = {}
        if len(elements): 
    ...
    def ID_container(self, ID):
        if "NMR" in ID: return "NMR tube"
        if "ICP" in ID: return "ICP tube"
        return "glass vial"
    ...
    def add(self, ID, 
        ...
        if component_ID:
            if component_ID in self.samples: 
                ...
                self.samples[ID]["composition"] =  self.add_composition(ID, component_ID, volume)
                self.samples[ID]["constitution"] = self.add_constitution(ID, component_ID, volume)
            ...
            if name: 
                ...
                self.samples[ID]["constitution"] = self.source_constitution(elements, concentrations)
                flag = 1
            else:
    ...
    def aliquot(self, ID, volume):
        if ID in self.samples: 
            ...
            if v: # if finite source
                ...
                if r<=0:
                    print("*** Improbably large aliquote for ID = %s, volume=%.2f uL, requested %.2f uL => skip" % (ID, v, volume))
                    return     
            
                f = r/v
                self.samples[ID]['parent_volumes'] = [x * f for x in self.samples[ID]['parent_volumes']]
...
########################################## itemization ####################################################  
    ...
    def extract_substrate(self, name, category=""): # create a bill of items on a substrate by plate name and category
        s = {}
        for ID in self.samples:
    ...
    def waste_bill(self, obj):
        ...
        u = Counter(u)
        l =  ["container","container count", "total volume, mL",] + sources
        rows = []
    ...
    def compositions2df(self, data, suffix): # takes a list of compositions and makes a table of it
    ...
    def constitutions2df(self, data): # takes a list of  concentrations and makes a table of it
        df = pd.DataFrame(data)
        df = df.reindex(columns=df.columns.union(df.columns), fill_value=np.nan)
...
def example():
    ...
    co.add("source2", name="reagent1", elements=["Li", "Na", "Ca"], concentrations=[10, 5, 8]) # mM default
    co.add("source3", name="reagent2", elements=["Li", "Fe", "Cr"], concentrations=[2, 1, 0.6])
