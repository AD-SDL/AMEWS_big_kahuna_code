...
class CustomAS10: # v is verbosity
	...
	def __init__(self, verbosity):
		...
		self.do_record = 1  # to record status statements, 0 - none, 1 - to a file, 2 - also print
		...
		self.next = None 
		self.last = None
		self.was_aborted = False # whether the experiment was aborted before the completion
		...
		self.normal_response = json.loads('{"Status":"Success","Content":"Experiment running","Error":"","StatusCode":0}')
		self.normal_active = json.loads('{"Status":"Success","Content":"No prompts are waiting for user input.","Error":"","StatusCode":1}')
	...
	def discover(self, name):
		...
		while(1):
			try:
				...
				print("SILA client discovery exception for client <%s>:\n	 %s" % (name,self.discovery))
	...
	def StartAR(self):
		...
		if self.discover("AutomationRemote"): return 1 # failed
		if self.client: 
	...
	def StartAS(self):
		...
		if self.discover("AutomationStudio"): return 1 #failed
		if self.client: 
	...
	def FindOrStartAS(self):
		if self.StartAR(): return 0 # failed
		if not self.checkResult(self.start):
			print("--- Not getting the expected response after discovery of AutomationRemote, waiting 20 s")
			time.sleep(20)  # changed from 20 s by UL
		if self.StartAS(): return 0 # failed
		else: return 1 # succeeded
	...
	def CloseAS(self):
		...
		for client in self.clients:
			self.client.close()
			if self.verbose: 
				print("Closed SILA client at %s, port %d" % (client.address,client.port))	
	...
	def GetStatusContent(self):
		try: 
			self.response = json.loads(self.client.ExperimentStatusService.GetStatus().ReturnValue)
		...
		if resume:
			...
			if chemfile: pass
				...
				if 'WithDC' not in promptsfile:
					print("ERROR: Design Studio designs require promptsWithDC.xml input")
					return "no-go"
				else:
			...
			if self.RunAS(design_ID, promptsfile, chemfile, tipfile): return "no-go"
		...
		while True:
			...
			if self.next != self.wait:
				...
				if self.last == self.no_tips:
					...
					print("CAUTION: Active prompt state <%s> needs input: %s" % (self.title,self.info))
					...
					print("CAUTION: Paused state <%s> needs input: %s" % (self.title,self.info))
					...
					print("ALERT: The experiment has been aborted, wait until it is completed\n")
